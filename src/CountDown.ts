interface CountDownProps {
  ...
  endTime?: number;
  auto: boolean;
  onInterval?: (timePassed: number, hour?: string, minute?: string, second?: string) => string;
  onEnd?: (timeLeft: number) => void;
}

class CountDown {
  private countType: 'seconds' | 'date';
  ...
  step: number;
  counter: number;
  private onInterval?: (timePassed: number, hour?: string, minute?: string, second?: string) => void;
  private onEnd?: (timeLeft: number) => void;
  ...
  // 位数补全
  static ten(t: number): string {
    return t < 10 ? '0' + t : t + '';
  }

  // 设置数据
  setData(props: CountDownProps): void {
    ...
    if ((this.timeLeft && (this.endTime || this.startTime)) || (!this.timeLeft && !(this.endTime || this.startTime))) {
    ...
    if (!this.timeLeft) {
      this.timeLeft = Math.floor(CountDown.getSeconds(this.endTime as number) - CountDown.getSeconds(this.startTime as number));
    }
